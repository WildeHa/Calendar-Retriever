...
public class CalendarReader {
	...
	static FileRetriever fr = new FileRetriever();

	// private static String[][] publicHoliday = { { "0101", "New Year's Day",
	// "一月一日", "元旦" },
	// { "0214", "Valentine's Day", "情人節", "情人节" }, { "0401", "April Fool's
	// Day", "愚人節", "愚人节" },
	// { "0404", "Children's Day", "兒童節", "儿童节" }, { "0501", "Labour Day",
	// "勞動節", "劳动节" },
	// { "0701", "Hong Kong Special Administrative Region Establishment Day",
	// "香港特別行政區成立紀念日", "香港特别行政区成立纪念日" },
	// { "1001", "National Day of the People's Republic of China", "國慶日", "国庆节"
	// },
	// { "1225", "Christmas Day", "聖誕節", "圣诞节" }, { "1226", "Boxing Day", "節禮日",
	// "节礼日" },
	// { "1231", "New Year's Eve", "新年前夕", "新年前夕" } };
	//
	// private static String[][] lunarHoliday = { { "0101", "Chinese Lunar New
	// Year's Day", "農曆年初一", "农历年初一" },
	// { "0102", "Second day of Chinese Lunar New Year", "農曆年初二", "农历年初二" },
	// { "0103", "Third day of Chinese Lunar New Year", "農曆年初三", "农历年初三" },
	// { "0505", "Dragon Boat Festival", "端午節", "端午节" }, { "0815", "MidAutumn
	// Festival", "中秋節", "中秋节" },
	// { "0909", "Double Ninth Festival", "重陽節", "重阳节" }, { "0408", "Buddha's
	// Birthday", "佛誕節", "佛诞节" },
	// { "0115", "Lantern Festival", "元宵節", "元宵节" }, { "0707", "Chinese
	// Valentine's Day", "七夕情人節", "七夕情人节" },
	...
	public static LinkedHashMap<String, ArrayList<String>> initHardCode(ArrayList<String> str) {

		LinkedHashMap<String, ArrayList<String>> hardMap = new LinkedHashMap<String, ArrayList<String>>();
		for (int i = 0; i < str.size(); i++) {
			String[] line = str.get(i).split(",", 2); // limit the number of
														// substring after
														// separating

			String[] holidays = line[1].split(","); // separate substrings into
													// holidays to put into a
													// new String[] after date
													// is return;

			for (int j = 0; j < holidays.length; j++) {
				holiday.add(holidays[j]);
			}
			hardMap.put(line[0], holiday);
	...
	public static LinkedHashMap<String, ArrayList<String>> initPulicHoliday(String date) {
		...
		// LinkedHashMap<String, ArrayList<String>> daHoMap =
		// initHardCode(publicHoliday);
		LinkedHashMap<String, ArrayList<String>> daHoMap = initHardCode(fr.getPubStr());
		...
		String easterDate = getEastSunDay(date, goMap);
		...
		String monDate = getEastMonDay(date, goMap);
		ArrayList<String> monList = new ArrayList<String>();
		monList.add("Easter Monday");
		monList.add("復活節星期一");
		monList.add("复活节星期一");
		daHoMap.put(monDate, monList);

		String saDate = getHoSatuDay(date, goMap);
		ArrayList<String> saList = new ArrayList<String>();
		saList.add("Holy Saturday");
		saList.add("聖週六");
		saList.add("圣周六");
		daHoMap.put(saDate, saList);

		String fiDate = getGoFriDay(date, goMap);
		ArrayList<String> friList = new ArrayList<String>();
		friList.add("Good Friday");
		friList.add("聖週五");
		friList.add("圣周五");
		daHoMap.put(fiDate, friList);
		...
		String dzDate = getDzDay(date);
		ArrayList<String> dzList = new ArrayList<String>();
		dzList.add("Winter Solstice ");
		dzList.add("冬至");
		dzList.add("冬至");
		daHoMap.put(dzDate, dzList);
	...
	public static LinkedHashMap<String, ArrayList<String>> initLunarHoliday(String date) {
		...
		LinkedHashMap<String, ArrayList<String>> lunarMap = initHardCode(fr.getLunStr());
	...
	public static String getEastSunDay(String date, LinkedHashMap<String, String> map) {
	...
	public static String getEastMonDay(String date, LinkedHashMap<String, String> map) {

		String year = date.substring(0, 4);
		String result = new String();
		for (String str : map.keySet()) {
			String value = map.get(str);
			if (value.contains("Easter Monday")) {
				if (str.contains(year)) {
					result = str;
				}
			}
	...
	public static String getGoFriDay(String date, LinkedHashMap<String, String> map) {

		String year = date.substring(0, 4);
		String result = new String();
		for (String str : map.keySet()) {
			String value = map.get(str);
			if (value.contains("Good Friday")) {
				if (str.contains(year)) {
					result = str;
				}
			}
	...
	public static String getHoSatuDay(String date, LinkedHashMap<String, String> map) {

		String year = date.substring(0, 4);
		String result = new String();
		for (String str : map.keySet()) {
			String value = map.get(str);
			if (value.contains("Holy Saturday")) {
				if (str.contains(year)) {
					result = str;
				}
			}
	...
	public static String getDzDay(String date) {
		...
		int day = SolarTermsUtil.principleTerm(Integer.parseInt(year), 12);

		StringBuffer sb = new StringBuffer();
		sb.append(year).append("12").append(Integer.toString(day));
